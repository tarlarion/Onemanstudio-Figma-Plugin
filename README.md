...
2. Scan-time `getStylesAndVariables`: local paint/text styles, paint styles used
   on **currently loaded pages**, text styles used on the current page, plus local
...
- **Parse components** logic remains, but the Parse button, debug output, and
  ...
  - Unlike detached replace, local→library does **not** copy text contents.
...
- Imports accept:
  - CSV with a `value` column and at least one of `id` or `key`. Header names
    are matched case-insensitively. Empty lines are dropped. Extra columns
    (`layerName`) are ignored.
  - JSON array, or object with an `items` array. Exported `frameId` is ignored.
    Import maps only `id` / `key` / `value`.
...
- **Generate semantic keys** asks the provider for lower-snake-case resource
  ...
  de-duplicated with `_2`, `_3`, … Empty sanitized keys become `text_key`.
...
- OpenRouter, OpenAI, and DeepSeek receive an OpenAI-compatible request with a
  ...
  prompt-enforced JSON plus fence stripping instead. Claude image input is a
...
| `replaceDetached` | `replaceDetachedResult`, `replaceDetachDebug` | Replaces selected detached nodes. Errors do not abort the batch. |
| `replaceLocalWithLibrary` | `replaceLocalWithLibraryResult` | File-wide instance swap; removes originals before import; no text copy (UI path currently hidden). |
...
- **CSV import rejected.** The file needs a `value` column and either `id` or
  `key`. Headers are case-insensitive; empty lines are ignored.
...
- Local→library replace is destructive on import failure; do not expose that
  path without restoring the original instance on error.
